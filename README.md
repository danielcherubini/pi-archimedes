...
6. **Optional — tune a keybinding** to `~/.pi/agent/keybindings.json` — Pi's [keybindings docs](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/keybindings.md) cover the format with examples. One worth doing on Linux early: clear Pi's built-in `app.clipboard.pasteImage` so [image-paste](packages/image-paste/README.md#paste-shortcuts) can take `Ctrl+V` by itself (that README has the two-line snippet).
