...
- Final: full repo gate — tsc in all 11 valid dirs (10 packages + meta, NOT repo root) + vitest in all 8 test packages
...
- [ ] Extend `packages/core/src/settings-io.test.ts` with failing tests:
  - `removeConfig` deletes an existing namespace key and leaves sibling keys intact; no-op (and no file rewrite if the key is absent — assert via mtime or spy)
...
- Test: `packages/notify/src/default-export.test.ts` (check for `enabled` export assertions — adjust if present), `meta/src/plugins.test.ts` only if notify/session-name test items are asserted there
- Modify: `README.md` — plugin-manager section: switches live in each package's own namespace (`archimedes.<pkg>.enabled`); drop the `archimedes.plugins` settings-table row
...
- `NotifyConfig` interface: `enabled: boolean` → `enabled?: boolean` with a comment: `// suite-managed by meta's plugin gate (archimedes.*  → see ADR 0012); notify never reads this`
...
- [ ] Grep: `grep -rn 'enabled' packages/notify/src packages/session-name/src meta/src | grep -v test` — enumerate every read; classify (suite-gate = keep only in meta/plugins.ts; runtime-guard/item = delete).
...
- [ ] Update README.md + AGENTS.md per above.
- [ ] Full repo gate: `npx tsc --noEmit` in ALL 11 valid dirs (10 packages + meta, not repo root); `npx vitest run` in all 8 test packages (core, ask, footer, diff, mcp, subagent, todo, meta).
...
- [ ] README has no `archimedes.plugins` row; AGENTS.md documents the suite-managed key + namespace in the checklist
- [ ] Full repo gate green; single commit
...
- Since Tasks 1 and 2 modify different packages, run each package's own suite before committing that task; the full 11-dir/8-suite gate is Task 3's final step.
