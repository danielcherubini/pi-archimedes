...
1. **New module** `packages/image-paste/src/keybinding-offer.ts` keeps `index.ts` slim. It exports one function: `offerKeybindingFix(ctx)` guarded end-to-end (gates in this order):
   ...
   - **Yes** → write `keybindings.json` with its own tmp+rename — **re-check `existsSync` immediately before rename** so a concurrently created file is never clobbered; content exactly the docs snippet (JSON with the `app.clipboard.pasteImage` key mapped to an empty array — i.e. `app.clipboard.pasteImage: []`). **Then** set the flag; then emit `CREATED_NOTIFY`; then call `await ctx.reload()` — the /reload flow re-reads keybindings.json and re-binds extension shortcuts so the cleared built-in binding applies immediately. Stale-ctx constraint: `ctx.reload()` must be the last use of ctx — the reload invalidates this extension instance. A failing reload is swallowed (file + flag already persisted; a manual /reload heals it; the flag gate blocks re-offer). File write strictly before flag set — deliberate, so a failed write leaves both gates open and the offer self-heals next session.
...
- Auto-reload: yes path calls `ctx.reload()` exactly once and only after the flag is persisted (last ctx use); no/cancel/TOCTOU/write-fail paths do not call `ctx.reload()`; a failing reload is swallowed and the offer still resolves without throwing.
